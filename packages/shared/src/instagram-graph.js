...
export async function fetchIGMedia(externalId, accessToken, limit = 20) {
    const data = await graphFetch(`/${externalId}/media`, accessToken, {
...
export async function fetchIGStories(externalId, accessToken) {
    const data = await graphFetch(`/${externalId}/stories`, accessToken, {
        fields: "id,media_type,media_product_type,media_url,thumbnail_url,caption,timestamp,permalink",
    });
    return data.data ?? [];
}
/** Fetch account-level insights */
export async function fetchIGInsights(externalId, accessToken, metrics = ["reach", "profile_views", "impressions", "accounts_engaged"], period = "day", since, until) {
    ...
    if (lifetimeMetricsToFetch.length > 0) {
        const lifetimeParams = {
            metric: lifetimeMetricsToFetch.join(","),
            period: "lifetime",
            metric_type: "total_value",
        };
        if (since)
            lifetimeParams.since = since;
        if (until)
            lifetimeParams.until = until;
        try {
            const lifetimeData = await graphFetch(`/${externalId}/insights`, accessToken, lifetimeParams);
    ...
    if (timeSeriesMetricsToFetch.length > 0) {
        ...
        if (since)
            timeParams.since = since;
        if (until)
            timeParams.until = until;
        try {
            const timeData = await graphFetch(`/${externalId}/insights`, accessToken, timeParams);
            if (timeData.data) {
...
export async function fetchIGProfile(externalId, accessToken) {
    return graphFetch(`/${externalId}`, accessToken, {
...
export async function publishIGPost(externalId, accessToken, imageUrl, caption) {
...
export async function searchHashtags(externalId, accessToken, hashtagName) {
    const data = await graphFetch("/ig_hashtag_search", accessToken, {
        user_id: externalId,
        q: hashtagName,
    });
    return data.data ?? [];
...
export async function getHashtagRecentMedia(hashtagId, accessToken, limit = 25) {
    const data = await graphFetch(`/${hashtagId}/recent_media`, accessToken, {
...
export async function fuzzySearchIGUsers(externalId, accessToken, query) {
...
export async function searchIGUser(externalId, accessToken, targetUsername) {
    const res = await graphFetch(`/${externalId}`, accessToken, {

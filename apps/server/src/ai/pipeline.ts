...
export async function analyzeSentiment(text: string) {
  ...
  try {
    ...
    const data = await geminiFetch("gemini-1.5-flash-latest", "generateContent", {
...
Generate a reply based on the user's message. Be concise (max 2 sentences), friendly, and professional.`;
    ...
    if (chatHistory.length > 0) {
      ...
      data = await geminiFetch("gemini-1.5-flash-latest", "generateContent", {
        contents,
...
export async function trackAnalytics(params: {
  ...
  const { db, analyticsEvents } = await import("@autoloop/db");

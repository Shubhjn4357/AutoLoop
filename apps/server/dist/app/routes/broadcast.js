    ...
    try {
        ...
        if (senderIds && senderIds.length > 0) {
            targetContacts = await db.query.contacts.findMany({
                where: and(eq(contacts.userId, userId), eq(contacts.externalId, account.externalId), inArray(contacts.senderId, senderIds)),
            });
        }
        else if (tag) {
            ...
            if (contactIds.length > 0) {
                targetContacts = await db.query.contacts.findMany({
                    where: and(eq(contacts.userId, userId), eq(contacts.externalId, account.externalId), inArray(contacts.id, contactIds)),
                });
            }
        }
        else {

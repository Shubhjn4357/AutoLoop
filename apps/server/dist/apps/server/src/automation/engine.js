import { db, eq, and, desc, automations, socialAccounts, messages as dbMessages, contacts, scheduledMessages, eventQueue, rateLimitState, aiConversations, automationMetrics, notificationLogs } from "@autoloop/db";
import { matchesAutomationCondition, sendInstagramMessage, replyToInstagramComment, getInstagramUserProfile } from "@autoloop/shared";
...
export const automationEngine = {
    ...
    async handleDM(payload, eventType) {
        ...
        const rules = await db.query.automations.findMany({
            where: and(eq(automations.userId, account.userId), eq(automations.triggerType, eventType), eq(automations.isActive, true)),
            orderBy: [desc(automations.priority)],
        });
        for (const rule of rules) {
            if (rule.targetPostId && rule.targetPostId !== payload.storyId)
                continue;
            if (!matchesAutomationCondition(rule.conditionOperator || 'contains', rule.condition || '', text))
                continue;
    ...
    async handleComment(payload) {
        ...
        if (!account?.accessToken)
            return;
        const rules = await db.query.automations.findMany({
            where: and(eq(automations.userId, account.userId), eq(automations.triggerType, "comment"), eq(automations.isActive, true)),
            orderBy: [desc(automations.priority)],
        });
        for (const rule of rules) {
            if (rule.targetPostId && rule.targetPostId !== mediaId)
                continue;
            if (!matchesAutomationCondition(rule.conditionOperator || 'contains', rule.condition || '', text))
                continue;
    ...
    async handleFollow(payload) {
        ...
        if (!account?.accessToken)
            return;
        const rules = await db.query.automations.findMany({
            where: and(eq(automations.userId, account.userId), eq(automations.triggerType, "follow"), eq(automations.isActive, true)),
            orderBy: [desc(automations.priority)],
        });
        for (const rule of rules) {
    ...
    async handleMention(payload) {
        const { externalId, senderId, text, commentId, mediaId } = payload;
        const account = await db.query.socialAccounts.findFirst({ where: eq(socialAccounts.externalId, externalId) });
        if (!account?.accessToken)
            return;
        const rules = await db.query.automations.findMany({
            where: and(eq(automations.userId, account.userId), eq(automations.triggerType, "mention"), eq(automations.isActive, true)),
            orderBy: [desc(automations.priority)],
        });
        for (const rule of rules) {
            if (rule.targetPostId && rule.targetPostId !== mediaId)
    ...
    async executeAutomation(rule, account, recipientId, messageText, messageId, commentId) {
    ...
    async checkRateLimits(externalId, recipientId, cooldownMinutes) {
    ...
    async interpolateVariables(text, userId, externalId, recipientId) {
    ...
    async upsertContact(userId, externalId, senderId, accessToken) {
    ...
    async scheduleFollowUp(userId, automationId, externalId, recipientId, template, delayMinutes) {
    ...
    async trackAutomationMetrics(userId, automationId) {
    ...
    async processScheduledMessage(msgId) {
